...
def plotAverageTimeSeries(samplesDict, ylabel, outFile, title=None,
	...
	xscale='lin', yscale='lin', margins=0.1):
	...
	for tsname, tssamples in samplesDict.items():
		...
		if disableStrips:
			...
			plotFunc(time, lower, time, upper, color=colors[colorIdx], alpha=0.5)
			plt.fill_between(time, lower, upper, where=upper>=lower, facecolor=colors[colorIdx], alpha=0.3, interpolate=True)
			plotFunc(time, tsavg, color=colors[colorIdx], label=tsname)

		colorIdx += 1
	limits = _applyMargins(yscale, limits, margins)
	_applyCosmetics(title, xlabel, ylabel, [0, xlimit] if xlimit else None, [0, ylimit] if ylimit else limits, legendLocation)
...
def plotAllTimeSeries(samplesDict, ylabel, outFile, title=None, xlabel='Time',
	...
	for tsname, tssamples in samplesDict.items():
		...
		plotFunc(time, tssamples[0,:], color=colors[colorIdx], alpha=alpha, label=tsname)
		for trajIdx in range(1, tssamples.shape[0]):
			plotFunc(time, tssamples[trajIdx,:], color=colors[colorIdx], alpha=alpha)
		colorIdx += 1
	limits = _applyMargins(yscale, limits, margins)
	_applyCosmetics(title, xlabel, ylabel, [0, xlimit] if xlimit else None, [0, ylimit] if ylimit else limits, legendLocation)
	fig = matplotlib.pyplot.gcf()
